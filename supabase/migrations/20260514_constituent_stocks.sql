-- Nifty 50 constituent stocks - static list with weights and sector
create table if not exists nifty_constituents (
  symbol        text primary key,
  upstox_key    text not null,
  company_name  text not null,
  sector        text not null,
  nifty_weight  numeric(5,2) not null,
  active        boolean default true
);

-- Daily OHLC data for each constituent stock
...
-- Top 20 Nifty 50 constituents by weight (May 2026)
