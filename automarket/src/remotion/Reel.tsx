...
export interface ReelProps {
  ...
  subtitleStyle?: 'bold' | 'minimal' | 'highlight';
  hookFrames:       number;
...
const DEF: Partial<ReelProps> = {
  brandColor: '#7c3aed', brandName: 'GodParticle',
  subtitleStyle: 'bold', hookFrames: 90, framesPerPhoto: 90, framesPerPoint: 75, ctaFrames: 90,
};
...
export function ReelComposition(raw: ReelProps) {
  ...
  segments.push({
    from: cursor, durationInFrames: p.hookFrames,
    el: <HookSlide hook={p.hook} brandColor={p.brandColor} />,
  });
  cursor += p.hookFrames;
  ...
  for (let i = 0; i < totalContentSlides; i++) {
    ...
    if (isPoint) {
      ...
      segments.push({
        from: cursor, durationInFrames: p.framesPerPoint,
        el: <TextSlide heading={points[ptIdx]} index={ptIdx + 1} brandColor={p.brandColor} totalFrames={p.framesPerPoint} />,
      });
      ...
      if (images.length > 0) {
        segments.push({
          from: cursor, durationInFrames: p.framesPerPhoto,
          el: <PhotoSlide imageUrl={images[imgIdx]} overlayText={points[imgIdx] ?? undefined}
                kenBurnsDir={KB_DIRS[imgIdx % 4]} totalFrames={p.framesPerPhoto} brandColor={p.brandColor} />,
        });
  ...
  segments.push({
    from: cursor, durationInFrames: p.ctaFrames,
    el: <CTASlide cta={p.cta} brandName={p.brandName} brandColor={p.brandColor} />,
  });
    ...
    <AbsoluteFill style={{ background: '#09090f', fontFamily: 'Arial, sans-serif' }}>
      {segments.map((seg, i) => (
        <Sequence key={i} from={seg.from} durationInFrames={seg.durationInFrames}>
          {seg.el as React.ReactElement}
        </Sequence>
      ))}
      {p.audioUrl && <Audio src={p.audioUrl} />}
      {p.bgMusicUrl && <Audio src={p.bgMusicUrl} volume={0.08} />}
      {p.subtitleWords.length > 0 && (
        <SubtitleOverlay words={p.subtitleWords} style={p.subtitleStyle} wordsPerChunk={3} />
      )}
    </AbsoluteFill>
  );
}
