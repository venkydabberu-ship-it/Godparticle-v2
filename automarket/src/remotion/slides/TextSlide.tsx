...
interface Props {
  heading: string; subtext?: string; emoji?: string;
  index?: number; brandColor?: string; totalFrames: number;
}
        ...
        {(index > 0 || emoji) && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 20, marginBottom: 32, transform: `scale(${numberScale})`, transformOrigin: 'left center' }}>
            {emoji ? (
              ...
              <div style={{ width: 80, height: 80, borderRadius: '50%', background: `linear-gradient(135deg, ${brandColor}, #f059da)`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'Arial Black', fontWeight: 900, fontSize: 44, color: '#fff' }}>
                {index}
              </div>
            )}
          </div>
        )}
        <h2 style={{ fontFamily: 'Arial Black, Arial, sans-serif', fontWeight: 900, fontSize: heading.length < 40 ? 80 : heading.length < 70 ? 64 : 52, color: '#f1f5f9', lineHeight: 1.15, margin: '0 0 32px 0', letterSpacing: '-1px' }}>
          {heading}
        </h2>
        {subtext && (
          <p style={{ fontFamily: 'Arial, sans-serif', fontSize: 40, color: '#94a3b8', lineHeight: 1.5, margin: 0, maxWidth: 900 }}>{subtext}</p>
        )}
