...
interface Props { hook: string; brandColor?: string; }

export function HookSlide({ hook, brandColor = '#7c3aed' }: Props) {
    ...
    <AbsoluteFill style={{ background: '#09090f', justifyContent: 'center', alignItems: 'center', padding: 60 }}>
        ...
        {words.map((word, i) => {
          ...
          return (
            <span key={i} style={{
              ...
              fontSize: hook.length < 40 ? 100 : hook.length < 80 ? 76 : 60,
              lineHeight: 1.1,
              color: isAccent ? brandColor : '#f1f5f9',
              transform: `translateY(${y}px)`, opacity,
              display: 'inline-block', letterSpacing: '-1px',
              textShadow: isAccent ? `0 0 40px ${brandColor}88` : 'none',
            }}>{word}</span>
          );
        })}
      </div>
      {frame > fps * 2 && (
        <div style={{
          ...
          opacity: interpolate(frame, [fps * 2, fps * 2.5], [0, 0.6]),
          color: '#64748b', fontFamily: 'Arial', fontSize: 28, letterSpacing: 4,
        }}>▼ ▼ ▼</div>
      )}
    </AbsoluteFill>
  );
