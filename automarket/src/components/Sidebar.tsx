  ...
  return (
    <aside className="fixed left-0 top-0 h-screen w-56 flex flex-col border-r border-[#1e1e2e] bg-[#0d0d14] z-40">
          ...
          return (
            <Link key={item.href} href={item.href}
              className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all
                ${active ? 'bg-[#7c3aed22] text-[#a78bfa] border border-[#7c3aed33]' : 'text-[#64748b] hover:text-[#f1f5f9] hover:bg-[#ffffff08]'}`}>
      ...
      <div className="px-4 py-4 border-t border-[#1e1e2e] flex flex-col gap-2">
        <PlatformDot label="Instagram" />
        <PlatformDot label="YouTube" />
      </div>
    </aside>
  );
}
...
function QueueBadge() {
  return <span className="ml-auto text-[10px] font-black px-1.5 py-0.5 rounded-full" style={{ background: '#f59e0b22', color: '#fbbf24' }} id="queue-badge" />;
}
