...
export async function POST(req: NextRequest) {
  try {
    const body = await req.json() as { contentId: string; idea: string; platform: Platform; contentType: ContentType; tone: Tone; };
...
export async function PATCH(req: NextRequest) {
  try {
    const body = await req.json() as { contentId: string; section: string; currentContent: Record<string, unknown>; idea: string; tone: Tone; };
    const result = await regenerateSection(body.section as never, body.currentContent as never, body.idea, body.tone);
    const db = supabaseAdmin();
    await db.from('am_content').update({ [`ai_${body.section}`]: result, updated_at: new Date().toISOString() }).eq('id', body.contentId);
