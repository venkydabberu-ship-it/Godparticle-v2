...
interface WebhookChange { field: string; value: { text?: string; from?: { id: string; username?: string }; id?: string; media_id?: string; }; }
...
async function fireAction(rule: AmAutomationRule, commentId: string, userId: string, message: string, db: ReturnType<typeof supabaseAdmin>, triggerData: object) {
  ...
  let success = true; let errorMsg: string | undefined;
  try {
