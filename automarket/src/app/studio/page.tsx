  ...
  async function handleGenerate() {
    ...
    try {
      ...
      try {
        ...
        setError('Server timeout or network error. Please try again.');
        setStep('input');
        return;
      }
  ...
  async function handleApprove() {
    ...
    try {
      const res = await fetch('/api/generate', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ contentId, caption: generated.caption, hashtags: generated.hashtags }),
      });
            ...
            <div className="flex flex-col gap-4">
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={4}
                  className="w-full bg-[#111118] border border-[#1e1e2e] rounded-xl p-3 text-sm text-[#f1f5f9]
                    placeholder-[#3f3f5a] resize-none focus:outline-none focus:border-[#7c3aed66]"
                />
              </div>
            </div>
                  ...
                  {PLATFORMS.map(p => (
                    <button key={p.value} onClick={() => setPlatform(p.value)}
                      className={`flex-1 flex flex-col items-center gap-1 py-3 rounded-xl border text-xs font-bold transition-all
                        ${platform === p.value
                          ? 'border-[#7c3aed] bg-[#7c3aed22] text-[#a78bfa]'
                          : 'border-[#1e1e2e] text-[#64748b] hover:border-[#ffffff18]'}`}>
                    ...
                    <button key={ct.value} onClick={() => setContentType(ct.value)}
                      className={`py-2 px-3 rounded-xl border text-left transition-all
                        ${contentType === ct.value
                          ? 'border-[#7c3aed] bg-[#7c3aed22]'
                          : 'border-[#1e1e2e] hover:border-[#ffffff18]'}`}>
                      <div className={`text-xs font-bold ${contentType === ct.value ? 'text-[#a78bfa]' : 'text-[#f1f5f9]'}`}>
                        {ct.label}
                      </div>
                ...
                <div className="flex flex-col gap-1.5">
                  {TONES.map(t => (
                    <button key={t.value} onClick={() => setTone(t.value)}
                      ...
                      <div>
                        <div className={`text-xs font-bold ${tone === t.value ? 'text-[#a78bfa]' : 'text-[#f1f5f9]'}`}>
                          {t.label}
                        </div>
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="flex gap-3 mt-2">
              ...
              <button onClick={handleApprove} disabled={saving}
                ...
                {saving ? 'Saving...' : 'Approve → Move to Queue'}
              </button>
            </div>
          </div>
        )}
...
function ContentBlock({
  ...
  value: string; editMode: keyof GeneratedContent | null; editVal: string;
  regenLoading: keyof GeneratedContent | null;
  onEdit: () => void; onRegen: () => void;
  onEditVal: (v: string) => void; onSaveEdit: () => void; onCancelEdit: () => void;
  ...
  return (
    <div className="card">
      ...
      {isEditing ? (
        <EditInline value={editVal} onChange={onEditVal} onSave={onSaveEdit} onCancel={onCancelEdit} multiline />
      ) : (
        <p className={`text-sm text-[#94a3b8] leading-relaxed whitespace-pre-wrap
          ${mono ? 'font-mono text-xs bg-[#ffffff04] p-3 rounded-lg' : ''}`}>
          {value}
        </p>
      )}
    </div>
...
function ActionBtn({ icon, label, onClick, loading }: {
  ...
  return (
    <button onClick={onClick} disabled={loading}
      className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-xs text-[#64748b]
        hover:bg-[#ffffff08] hover:text-[#f1f5f9] transition-all font-medium disabled:opacity-50">
...
function EditInline({ value, onChange, onSave, onCancel, multiline }: {
    ...
    <div className="flex flex-col gap-2">
      {multiline ? (
        <textarea value={value} onChange={e => onChange(e.target.value)} rows={6}
          className="w-full bg-[#0d0d14] border border-[#7c3aed66] rounded-xl p-3 text-sm text-[#f1f5f9]
            resize-none focus:outline-none" autoFocus />
      ) : (
