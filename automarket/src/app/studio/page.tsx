...
export default function Studio() {
  ...
  async function handleGenerate() {
    ...
    try {
      // Call API — server handles DB insert + Claude generation
      const res = await fetch('/api/generate', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ idea: idea.trim(), platform, contentType, tone, imageUrls: [] }),
      });

      let json: { contentId?: string; content?: GeneratedContent; error?: string } = {};
      ...
      setContentId(json.contentId ?? null);
  ...
  function startEdit(section: keyof GeneratedContent) {
    if (!generated) return;
    const val = section === 'hashtags'
      ? (generated.hashtags as string[]).join(', ')
      : generated[section] as string;
    setEditVal(val);
    setEditMode(section);
  }
  ...
  function saveEdit() {
    ...
    if (editMode === 'hashtags') {
      setGenerated({ ...generated, hashtags: editVal.split(',').map(h => h.trim().replace(/^#/, '')) });
    } else {
      setGenerated({ ...generated, [editMode]: editVal });
    }
    setEditMode(null);
  }
    ...
    <AppShell>
      <div className="p-8 max-w-4xl">
          ...
          {(['input', 'generating', 'review'] as Step[]).map((s, i) => (
            <div key={s} className="flex items-center gap-2">
              <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-black
                ${step === s ? 'bg-gradient-to-br from-[#7c3aed] to-[#f059da] text-white'
                  : i < ['input','generating','review'].indexOf(step) ? 'bg-[#10b981] text-white' : 'bg-[#1e1e2e] text-[#64748b]'}`}>
                {i < ['input','generating','review'].indexOf(step) ? '✓' : i + 1}
              </div>
              <span className={`text-xs font-medium capitalize ${step === s ? 'text-[#a78bfa]' : 'text-[#64748b]'}`}>
                {s === 'input' ? 'Your Idea' : s === 'generating' ? 'AI Writes It' : 'Review & Approve'}
              </span>
        ...
        {step === 'input' && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="flex flex-col gap-4">
              <div className="border-2 border-dashed border-[#1e1e2e] rounded-2xl p-6 text-center cursor-pointer hover:border-[#7c3aed66] hover:bg-[#7c3aed08] transition-all"
                onDragOver={e => e.preventDefault()} onDrop={onDrop} onClick={() => fileRef.current?.click()}>
                <input ref={fileRef} type="file" accept="image/*" multiple className="hidden"
                  onChange={e => addImages(Array.from(e.target.files ?? []))} />
                {imagePreviews.length === 0 ? (
                  <><div className="text-4xl mb-2">📸</div><div className="text-sm font-medium text-[#f1f5f9]">Drop your photos here</div><div className="text-xs text-[#64748b] mt-1">or click to browse · max 10 images</div></>
                ) : (
                  <div className="grid grid-cols-3 gap-2">
                    {imagePreviews.map((src, i) => (
                      <div key={i} className="relative aspect-square rounded-lg overflow-hidden">
                        <img src={src} alt="" className="w-full h-full object-cover" />
                        <button onClick={e => { e.stopPropagation(); const next = images.filter((_,j)=>j!==i); setImages(next); setImagePreviews(next.map(f=>URL.createObjectURL(f))); }}
                    ...
                    {imagePreviews.length < 10 && <div className="aspect-square rounded-lg border-2 border-dashed border-[#1e1e2e] flex items-center justify-center text-[#64748b] text-xl">+</div>}
                  </div>
                )}
              </div>
              <div>
                ...
                <textarea value={idea} onChange={e => setIdea(e.target.value)}
                  placeholder="e.g. Show how the God Particle predicts where market reverses every expiry..."
                  rows={4} className="w-full bg-[#111118] border border-[#1e1e2e] rounded-xl p-3 text-sm text-[#f1f5f9] placeholder-[#3f3f5a] resize-none focus:outline-none focus:border-[#7c3aed66]" />
              </div>
            </div>
                ...
                <div className="flex gap-2">
                  {PLATFORMS.map(p => (
                    <button key={p.value} onClick={() => setPlatform(p.value)}
                      className={`flex-1 flex flex-col items-center gap-1 py-3 rounded-xl border text-xs font-bold transition-all ${platform === p.value ? 'border-[#7c3aed] bg-[#7c3aed22] text-[#a78bfa]' : 'border-[#1e1e2e] text-[#64748b] hover:border-[#ffffff18]'}`}>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {CONTENT_TYPES.map(ct => (
                    <button key={ct.value} onClick={() => setContentType(ct.value)}
                      className={`py-2 px-3 rounded-xl border text-left transition-all ${contentType === ct.value ? 'border-[#7c3aed] bg-[#7c3aed22]' : 'border-[#1e1e2e] hover:border-[#ffffff18]'}`}>
                      <div className={`text-xs font-bold ${contentType === ct.value ? 'text-[#a78bfa]' : 'text-[#f1f5f9]'}`}>{ct.label}</div>
                      <div className="text-[10px] text-[#64748b]">{ct.note}</div>
                    </button>
                  ))}
                </div>
              </div>
                ...
                <div className="flex flex-col gap-1.5">
                  {TONES.map(t => (
                    <button key={t.value} onClick={() => setTone(t.value)}
                      className={`flex items-center gap-3 px-3 py-2.5 rounded-xl border text-left transition-all ${tone === t.value ? 'border-[#7c3aed] bg-[#7c3aed22]' : 'border-[#1e1e2e] hover:border-[#ffffff18]'}`}>
                      <span className="text-base">{t.emoji}</span>
                      <div>
                        <div className={`text-xs font-bold ${tone === t.value ? 'text-[#a78bfa]' : 'text-[#f1f5f9]'}`}>{t.label}</div>
        ...
        {step === 'generating' && (
          <div className="card flex flex-col items-center justify-center py-20 gap-6">
            <div className="relative">
              <div className="w-16 h-16 rounded-2xl flex items-center justify-center text-3xl" style={{ background: 'linear-gradient(135deg,#7c3aed,#f059da)' }}>🤖</div>
        ...
        {step === 'review' && generated && (
          <div className="flex flex-col gap-4">
            <ContentBlock icon="⚡" title="Hook (First 3 Seconds)" section="hook" value={generated.hook} editMode={editMode} editVal={editVal} regenLoading={regenLoading} onEdit={() => startEdit('hook')} onRegen={() => handleRegenSection('hook')} onEditVal={setEditVal} onSaveEdit={saveEdit} onCancelEdit={() => setEditMode(null)} />
            <ContentBlock icon="📝" title="Caption" section="caption" value={generated.caption} editMode={editMode} editVal={editVal} regenLoading={regenLoading} onEdit={() => startEdit('caption')} onRegen={() => handleRegenSection('caption')} onEditVal={setEditVal} onSaveEdit={saveEdit} onCancelEdit={() => setEditMode(null)} multiline />
            <div className="card">
              ...
              {editMode === 'hashtags' ? <EditInline value={editVal} onChange={setEditVal} onSave={saveEdit} onCancel={() => setEditMode(null)} /> : (
                <div className="flex flex-wrap gap-1.5">
                  {generated.hashtags.map((h, i) => <span key={i} className="text-xs px-2 py-1 rounded-full bg-[#7c3aed22] text-[#a78bfa] font-mono">#{h}</span>)}
                </div>
              )}
            </div>
            <ContentBlock icon="🎬" title="Video Script" section="script" value={generated.script} editMode={editMode} editVal={editVal} regenLoading={regenLoading} onEdit={() => startEdit('script')} onRegen={() => handleRegenSection('script')} onEditVal={setEditVal} onSaveEdit={saveEdit} onCancelEdit={() => setEditMode(null)} multiline mono />
            <ContentBlock icon="🎯" title="Call To Action" section="cta" value={generated.cta} editMode={editMode} editVal={editVal} regenLoading={regenLoading} onEdit={() => startEdit('cta')} onRegen={() => handleRegenSection('cta')} onEditVal={setEditVal} onSaveEdit={saveEdit} onCancelEdit={() => setEditMode(null)} />
            <div className="flex gap-3 mt-2">
              ...
              <button onClick={handleApprove} disabled={saving} className="flex-[2] btn-primary py-3 text-sm flex items-center justify-center gap-2">
                {saving ? <span className="spinner" /> : '✅'}{saving ? 'Saving...' : 'Approve → Move to Queue'}
              </button>
            </div>
          </div>
        )}
...
function ContentBlock({ icon, title, section, value, editMode, editVal, regenLoading, onEdit, onRegen, onEditVal, onSaveEdit, onCancelEdit, multiline, mono }: { icon: string; title: string; section: keyof GeneratedContent; value: string; editMode: keyof GeneratedContent | null; editVal: string; regenLoading: keyof GeneratedContent | null; onEdit: () => void; onRegen: () => void; onEditVal: (v: string) => void; onSaveEdit: () => void; onCancelEdit: () => void; multiline?: boolean; mono?: boolean; }) {
  ...
  return (
    <div className="card">
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2"><span className="text-base">{icon}</span><span className="font-bold text-sm text-[#f1f5f9]">{title}</span></div>
        <div className="flex gap-2">
...
function ActionBtn({ icon, label, onClick, loading }: { icon: string; label: string; onClick: () => void; loading?: boolean; }) {
  return (
    <button onClick={onClick} disabled={loading} className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-xs text-[#64748b] hover:bg-[#ffffff08] hover:text-[#f1f5f9] transition-all font-medium disabled:opacity-50">
      {loading ? <span className="spinner" style={{ width: 12, height: 12 }} /> : icon}{label}
    </button>
  );
}
