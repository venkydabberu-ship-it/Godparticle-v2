...
interface AccountInfo {
  display_name: string; handle: string; follower_count: number;
  following_count: number; media_count: number; last_synced_at: string | null;
}
      ...
      <div className="p-8 max-w-4xl">
        <div className="flex items-start justify-between mb-6">
          <div>
            ...
            <p className="text-[#64748b] text-sm mt-1">Account performance and post metrics</p>
          </div>
          <button onClick={handleSync} disabled={syncing} className="btn-primary text-sm px-4 py-2.5 flex items-center gap-2">
            {syncing ? <span className="spinner" /> : '🔄'} {syncing ? 'Syncing...' : 'Sync Account'}
          </button>
        </div>
        ...
        {loading ? (
          ...
          <>
            {ig ? (
              <div className="card mb-6 border-[#e1306c33]">
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <div>
                    <div className="font-black text-[#f1f5f9]">@{ig.handle}</div>
                    <div className="text-xs text-[#64748b]">{ig.last_synced_at ? `Last synced ${new Date(ig.last_synced_at).toLocaleString('en-IN', { dateStyle: 'short', timeStyle: 'short' })}` : 'Not yet synced'}</div>
                  </div>
                  <div className="ml-auto flex items-center gap-1.5">
                    <div className="w-2 h-2 rounded-full bg-[#10b981]" />
                    <span className="text-xs text-[#10b981] font-bold">Connected</span>
                  </div>
                </div>
              ...
              <div className="card mb-6 text-center py-8">
                ...
                <div className="text-[#f1f5f9] font-bold mb-1">Instagram not synced yet</div>
                <div className="text-[#64748b] text-sm mb-4">Click "Sync Account" to pull your stats</div>
              </div>
            )}
            ...
            <div className="card">
              ...
              {posts.length === 0 ? (
                <div className="text-center py-6 text-[#64748b] text-sm">No posted content yet</div>
              ) : (
                  ...
                  {posts.map((p, i) => (
                    <div key={i} className="flex items-center gap-3 py-2 border-b border-[#1e1e2e] last:border-0">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="text-xs text-[#f1f5f9] truncate">{p.ai_hook ?? 'Post'}</div>
                        <div className="text-[10px] text-[#64748b]">{p.posted_at ? new Date(p.posted_at).toLocaleDateString('en-IN', { day: 'numeric', month: 'short' }) : '—'}</div>
                      </div>
                      <div className="flex items-center gap-3 text-xs flex-shrink-0">
                        <span className="text-[#f059da]">❤️ {p.ig_like_count ?? 0}</span>
                        <span className="text-[#a78bfa]">💬 {p.ig_comment_count ?? 0}</span>
                      </div>
                    </div>
                  ))}
                </div>
