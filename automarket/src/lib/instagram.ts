...
export async function pollContainerStatus(creationId: string, maxWaitMs = 120_000): Promise<void> {
  ...
  while (Date.now() < deadline) {
    const data = await igFetch(
      `/${creationId}?fields=status_code,status&access_token=${TOKEN}`
...
export async function getMediaInsights(mediaId: string) {
  const fields = 'reach,impressions,likes,comments,shares,saved';
  return igFetch(`/${mediaId}/insights?metric=${fields}&access_token=${TOKEN}`);
}
