...
export async function publishContent(content: AmContent): Promise<void> {
  ...
  try {
    ...
    if (content.platform === 'instagram' || content.platform === 'both') {
      await publishToInstagram(content, caption, db);
    }
    if (content.platform === 'youtube' || content.platform === 'both') {
      await publishToYouTube(content, caption, db);
    }
...
async function publishToYouTube(c: AmContent, caption: string, db: ReturnType<typeof supabaseAdmin>) {
  ...
  const { videoId, url } = await uploadShort({
    videoUrl: c.video_url, title: hook.slice(0, 100),
    description: caption, tags: (c.final_hashtags ?? c.ai_hashtags ?? []),
  });
...
export async function processScheduledPosts(): Promise<{ processed: number; errors: string[] }> {
  ...
  const { data: due } = await db
    .from('am_content').select('*').eq('status', 'scheduled').lte('scheduled_at', now);

  if (!due?.length) return { processed: 0, errors: [] };

  const errors: string[] = [];
  for (const item of due) {
    try { await publishContent(item as AmContent); }
    catch (e) { errors.push(`${item.id}: ${e}`); }
  }
