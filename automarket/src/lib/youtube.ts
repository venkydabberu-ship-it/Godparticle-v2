...
export async function uploadShort(params: { videoUrl: string; title: string; description: string; tags: string[] }): Promise<{ videoId: string; url: string }> {
  const token = await getAccessToken();
  const videoRes = await fetch(params.videoUrl);
  const videoBuffer = await videoRes.arrayBuffer();
  const metadata = {
    snippet: { title: params.title.slice(0, 100), description: params.description.slice(0, 5000), tags: params.tags.slice(0, 500), categoryId: '22' },
  ...
  const combined = new Uint8Array(metaBytes.byteLength + videoTypeHeader.byteLength + videoBuffer.byteLength + closeBytes.byteLength);
  let offset = 0;
  ...
  const res = await fetch(`${YT_UPLOAD}/videos?uploadType=multipart&part=snippet,status`, {
    method: 'POST',
...
export async function getChannelStats() {
  ...
  const res = await fetch(`${YT_BASE}/channels?part=statistics,snippet&id=${channelId}`, { headers: { Authorization: `Bearer ${token}` } });
