...
export async function mergeAudio(videoPath: string, audioPath: string, outputPath?: string): Promise<string> {
  ...
  await run(ffmpeg().input(videoPath).input(audioPath).outputOptions(['-c:v copy','-c:a aac','-b:a 192k','-shortest','-movflags +faststart']).output(out));
  return out;
}

export async function burnSubtitles(videoPath: string, subsPath: string, style: 'bold' | 'minimal' | 'highlight' = 'bold', outputPath?: string): Promise<string> {
  const out = outputPath ?? tmpFile('mp4');
  ...
  const filterStr = ext === '.ass' ? `ass='${escapedSubs}'` : `subtitles='${escapedSubs}':force_style='${fontStyles[style]}'`;
  await run(ffmpeg().input(videoPath).videoFilter(filterStr).outputOptions(['-c:a copy','-movflags +faststart']).output(out));
  return out;
}

export async function mixBackgroundMusic(videoPath: string, musicPath: string, musicVol = 0.08, outputPath?: string): Promise<string> {
  const out = outputPath ?? tmpFile('mp4');
  await run(ffmpeg().input(videoPath).input(musicPath)
    .complexFilter([`[1:a]volume=${musicVol}[music]`,`[0:a][music]amix=inputs=2:duration=first:dropout_transition=2[aout]`])
...
export async function scaleVideo(inputPath: string, width = 1080, height = 1920, outputPath?: string): Promise<string> {
  const out = outputPath ?? tmpFile('mp4');
  await run(ffmpeg().input(inputPath)
...
export async function getVideoDuration(videoPath: string): Promise<number> {
  return new Promise((resolve, reject) => {
    ffmpeg.ffprobe(videoPath, (err, meta) => { if (err) return reject(err); resolve(meta.format.duration ?? 0); });
  });
}
