...
export async function buildSubtitles(audioBuffer: Buffer, existingWords?: WordTimestamp[]): Promise<{ srtContent: string; words: WordTimestamp[] }> {
  let words = existingWords;
  if (!words || words.length === 0) {
    try { words = await transcribeAudio(audioBuffer); } catch (e) { console.error('Whisper transcription failed:', e); words = []; }
  }
