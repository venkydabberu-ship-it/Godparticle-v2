...
create table if not exists am_automation_rules (
  ...
  name           text not null,
  platform       text not null check (platform in ('instagram', 'youtube', 'both')),
  trigger_type   text not null check (trigger_type in ('comment_keyword', 'dm_keyword', 'new_follower', 'story_reply', 'post_tag', 'reel_share')),
  trigger_keywords text[],
  match_mode     text default 'any' check (match_mode in ('any', 'all', 'exact')),
  action_type    text not null check (action_type in ('reply_comment', 'send_dm', 'follow_back', 'like_comment', 'send_link', 'tag_story')),
  action_message text,
...
('viral_hook', 'easy', '50K–200K', 'The Number Institutions Never Want You to Know',
...
('meme_funny', 'easy', '200K–1M', 'Me vs The Market (Every Expiry)',
...
('educational', 'medium', '100K–500K', 'Why You Were Right About Direction but Still Lost Money',
 E'You predicted the move correctly. You still lost money. Here''s why 👇',
...
('behind_scenes', 'medium', '80K–300K', 'Watch the Zero-to-Hero Signal Fire Live',
 E'It''s 11:30 AM Thursday. 5 forces just aligned. Watch what happens next 👀',
...
('trending_challenge', 'easy', '500K–2M', 'Tell Me You Trade Options Without Telling Me',
 E'Tell me you trade options without telling me you trade options 🙃',
...
('testimonial', 'easy', '30K–150K', 'What Happens When You Trade WITH Institutional Gravity',
 E'Nobody believes me until they see the chart themselves 📊',
